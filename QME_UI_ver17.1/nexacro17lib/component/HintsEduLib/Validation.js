...
pForm.gfn_setValidation = function(objDs, sColID, sColName, sRule)
{
	...
	//if (sColName.indexOf(",") != -1) sColName = nexacro.replaceAll(sColName, ",", "^_^");
										this.gtrace("this.gfn_isNull(objDs.validationRuleSet)-->"+this.gfn_isNull(objDs.validationRuleSet), gtrcPos);
...
pForm.gfn_validation = function()
{
	...
	var sCheckMode   = "U";
												this.gtrace("arguments.length--->"+arguments.length, gtrcPos);
												this.gtrace("arguments[0].name--->"+arguments[0].name, gtrcPos);
												this.gtrace("arguments[1]--->"+arguments[1], gtrcPos);
		...
		for(var i=0; i<objDs.getRowCount(); i++)
		{
			...
			// 전체 row
				...
				++nUpdateCnt;
																					...
																					//	this.gtrace("arrValidation.length--->"+arrValidation.length, gtrcPos);
			// Rule 만큼 처리
			for(var j=0; j<arrValidation.length; j++)
			{
				var sColID     = arrValidation[j].name;								if(j==0)this.gtrace("sColID--->"+sColID, gtrcPos);
				var sColName   = new String(arrValidation[j].title);				if(j==0)this.gtrace("sColName--->"+sColName, gtrcPos);
				var sColValue  = String(objDs.getColumn(i, sColID)).trim();			if(j==0)this.gtrace("sColValue--->"+sColValue, gtrcPos);

				var arrRule    = (""+arrValidation[j].value).split(","); //required,length:5
																					if(j==0)this.gtrace("arrRule--->"+arrRule, gtrcPos);
				var bNullCheck = arrValidation[j].notnull;
																					if(j==0)this.gtrace("bNullCheck--->"+bNullCheck, gtrcPos);
				// 필수입력 체크
				if(bNullCheck == true){		//required가 있으므로 nullCheck를 하라
																					if(j==0)this.gtrace("this.gfn_nvl(sColValue,'')--->"+this.gfn_nvl(sColValue,""), gtrcPos);

				//	if(this.gfn_nvl(sColValue,"")==""){	//공백인 경우도체크 undefined인 경우는 ??
					if(this.gfn_isNull(sColValue)){
																					if(j==0)this.gtrace("<---------------------->", gtrcPos);
						// row 이동 및 focus 저장
						...
						if (sColName.indexOf(",") != -1) {
							...
							titile = sColName;
																			...
																			//	this.gfn_setValidation(this.ds_data	, "COL_ID"  , "아이디" , "required,length:5");    
																			//								objDs	, sColID	, sColName	, sRule
																			if(j==0)this.gtrace("[titile]-------------->"+[titile], gtrcPos);	//	[titile]-------------->아이디
						// 메시지 처리 : ${} 필수 입력 항목입니다.
						...
						if(j==0)this.gtrace("gfn_alert실행------>msg.err.validator.required-->title:"+[titile], gtrcPos);	//	[titile]-------------->아이디
				...
				// 필수값이 아닌 경우는 체크할 값이 null이 아닌 경우에만 체크 <-- 코멘트가 틀린 듯.
				//		if(!this.gfn_isNull(sColValue)){
				if(this.gfn_nvl(sColValue,"")!=""){	
					// Rule에 따른 정합성 체크
					...
					for(var k=0; k<arrRule.length; k++)
					{
						...
						// 컬럼의 한개 값을 이용해서 처리하는 check 대상
											...
											if(j==2)this.gtrace("<--------컬럼의 한개 값을 이용해서 처리하는 check 대상------->"+checkrule, gtrcPos);
							msg = this.gfn_validationCheckRule(sColName, sColValue, arrRule[k]);
											if(j==2)this.gtrace("msg------->"+msg, gtrcPos);
						}
